...
Note: to handle the total, used as denominatory, categorical values the field denominatory is populated. Instead of count there could be a category called total and then denominator refering to that category? This is the "N[a]" item in the PhUSE table. 
```{r, eval=TRUE}
RDFCubeWorkbook<- system.file("extdata/sample-cfg", "RDFCubeWorkbook.xlsx", package="rrdfqbcrnd0")
cubeMetadata <- read.xlsx(RDFCubeWorkbook,
                          sheetName=paste0("DEMO-Components"),
                          stringsAsFactors=FALSE)
