...
Here is how to re-produce the metadata.
...
Secondly, we get the metadata.
The metadata element "cubeVersion" is not directly available but from dcat:distribution derived as
the result of paste0("DC-", domainName,"-R-V-",cubeVersion,".TTL").
...
cubeVersion<-  gsub("-",".", gsub("DC-.*-R-V-([^\\.]+).TTL", "\\1", cube.metadata[ cube.metadata[,2]=="distribution", "compLabel"], perl=TRUE))

cube.metadataX<- rbind(cube.metadata, 
rbind(cube.metadata, cbind(compType="metadata", compName="cubeVersion", compLabel=cubeVersion)))

knitr::kable(cube.metadataX)

```

Get the meta data from the excel workbook and show only the compName and value
