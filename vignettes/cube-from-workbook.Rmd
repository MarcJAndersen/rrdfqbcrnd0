...
## getting the column names (variable) names for the row dimensions
...
## XX need a function to determine variable (column names) from properties for a given language
colnamesRowDim<- sub("prop:", "", rowdim)
uqr<-data.frame(unique(observationsRowDim[,colnamesRowDim]))
## assuming rowno is a unused column name 
uqr[,"rowno"]<- 1:nrow(uqr)
observationsRowDimE<-merge(observationsRowDim, uqr, by=colnamesRowDim)
## assuming s is the name of the IRI
...
uqc<-data.frame(unique(observationsColDim[,colnamesColDim]))
uqc[,"colno"]<- 1:nrow(uqc)
observationsColDimE<-merge(observationsColDim,uqc)
...
obsRowColNo<- merge( observationsRowDimE[,c("s","rowno")], observationsColDimE[,c("s","colno")])
observationsDescX<- merge( observationsDesc, obsRowColNo, by="s")
rowcolOrder<- order( observationsDescX$rowno, observationsDescX$colno )
observationsDescXX<- observationsDescX[rowcolOrder,]
