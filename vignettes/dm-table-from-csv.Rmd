...
require(knitr)
knit("vignettes/demographics-table-from-csv.Rmd")
```

Generate the pdf file using the following command
```{r makepdf.sh, eval=FALSE, engine='bash'}
pandoc demographics-table-from-csv.md -o xx.pdf
```
...
```{r, eval=FALSE} 
knitr::knit2html("vignettes/demographics-table-from-csv.Rmd")
```
...
```{r, eval=FALSE}
knitr::knit2pdf("vignettes/demographics-table-from-csv.Rmd")
```
...
dataDir <- tempdir()  # Input and output files
...
DMskeletonSourceFile <- system.file("extdata/sample-cfg", "skeletonSource-DM.csv", package="rrdfqbcrnd0")

DMdsdName <- "dsd-demog"

# Cube Observation data       
DMobsFileName<- system.file("extdata/sample-cfg", "dm.AR.csv", package="rrdfqbcrnd0")
