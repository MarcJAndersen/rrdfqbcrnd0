...
## Prelimnaries
The present version uses a local version of the SDTM-Terminlogy file from 
rdf.cdisc.org. converted from OWL to TTL 
...
cubeVersion <- "0-5-3h"  # 0-5-3 using .XLSX for DM Skeleton  

sdtm.terminology.file = system.file("extdata/sample-cfg", 
                                    "sdtm-terminology-2013-06-28.ttl", 
                                    package="rrdfqbcrnd0")
sdtm.terminology= load.rdf(sdtm.terminology.file, format="N3")
# tempdir() not working on TW system. CHanged to getwd() + data subfolder
# dataOut <- tempdir()  # Input and output files: Used by MJA
dataOut <- paste0(getwd(), "/data/"  )  # Input and output files
...
RDFCubeWorkbook = system.file("extdata/sample-cfg", "RDFCubeWorkBook.xlsx", 
                              package="rrdfqbcrnd0")
if (! file.exists(RDFCubeWorkbook) ) {
     # consider using try instead
    stop( paste0("Expected file ", RDFCubeWorkbook, " does not exist" ))
}
...
# TODO: File name should become a parameter, not hard coded as DM.AR.CSV
# Naming files using domainName?  <domainName>.AR.CSV may be too restrictive
obsFile<- system.file("extdata/sample-cfg", "dm.AR.csv", package="rrdfqbcrnd0")

#TODO: Get the following from some other data - eg. stored in the cube?
recode.list= list(
...
dataCubeFile      <- paste0(dataOut,dataCubeFileName) # Full path to cube
