...
## Define data
The data are defined as data frame.
```{r}
obsData<- data.frame(
...
## Define meta data
The metadata are also defined as data frame.
```{r}
...
most often not be of interest, as the RRDF cube contain general
...
The next statement gets the first 30 triples in the cube, 
where the subject is a qb:Observation, and shows the first 15 triples.
```{r}
...
As RRDF does not support the ASK query, the number of triples selected
are returned.
